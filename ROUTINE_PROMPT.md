...
3. Publications. For each candidate in `publication_candidates.json`:
   - Drop it if it is not a journal article, book chapter, or book (for example a dataset, erratum, or peer review). Drop preprints unless the digest should mention them.
   ...
   - Score lab relevance from 1 to 5: does the paper study startups, founders, entrepreneurship, venture funding, or startup employees? Give one sentence of reasoning. Propose papers scoring 3 or more. List lower scores in the digest only.
...
6. UvA profiles. Do not change any data because of `uva_profiles.json`. Report flagged entries in the digest.
7. Low-risk updates. If `metadata_updates.json` lists updates, apply exactly those field changes in one branch `claude/auto-<date>`, open one pull request, and add the label `auto`. Change nothing else in that branch.
