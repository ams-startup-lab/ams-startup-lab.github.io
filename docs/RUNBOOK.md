...
The Network page is a logo wall of organisations the lab has worked with, past or present. Ongoing collaborations belong under Activities, as research projects. Organisations are never added by the routine. A member tells the director, who adds the partner by hand or opens a suggestion issue.

1. Ask the member to confirm with the organisation that the lab may name it on the website. For a logo, ask separately.
2. Add `data/network/<slug>.yaml` with `name`, `url`, `members` (member slugs), `group: uva` for UvA centres and labs (shown under "Our friends at the UvA"), and `confirmed` (the date of the member's confirmation).
3. For a logo, put a PNG with a transparent background in `public/network/` and add `logo: /network/<file>` and `logo_permission: <date>`. The build fails if a logo has no permission date.
...
2. Only public information. If the item names a company or client, the member first checks that they agree, as for the Network page.
